...
namespace EscapeFromTheWoods
{
    public class Wood
    {
        ...
        //async
        public async void AsyncEscape()
        {
            List<Task<List<Tree>>> routes = new List<Task<List<Tree>>>();
            foreach (Monkey m in monkeys)
            {
                routes.Add(AsyncEscapeMonkey(m));
            }
            //await Task.Run(() => AsyncWriteEscaperoutesToBitmap(routes));
            await Task.WhenAll(routes);
            AsyncWriteEscaperoutesToBitmap(routes);
        }
        private async void AsyncwriteRouteToDB(Monkey monkey, List<Tree> route)
        {
            Console.ForegroundColor = ConsoleColor.DarkGreen;
            Console.WriteLine($"ASYNC | {woodID}:write db routes {woodID},{monkey.name} start");
            List<DBMonkeyRecord> records = new List<DBMonkeyRecord>();
            for (int j = 0; j < route.Count; j++)
            {
                records.Add(new DBMonkeyRecord(monkey.monkeyID, monkey.name, woodID, j, route[j].treeID, route[j].x, route[j].y));
            ...
            db.WriteMonkeyRecords(records);
            Console.ForegroundColor = ConsoleColor.DarkGreen;
            Console.WriteLine($"ASYNC | {woodID}:write db routes {woodID},{monkey.name} end");
        }
        public async void AsyncWriteEscaperoutesToBitmap(List<Task<List<Tree>>> routes)
        {
            Console.ForegroundColor = ConsoleColor.Yellow;
            Console.WriteLine($"ASYNC | {woodID}:write bitmap routes {woodID} start");
            Color[] cvalues = new Color[] { Color.Red, Color.Yellow, Color.Blue, Color.Cyan, Color.GreenYellow };
            Bitmap bm = new Bitmap((map.xmax - map.xmin) * drawingFactor, (map.ymax - map.ymin) * drawingFactor);
            Graphics g = Graphics.FromImage(bm);
            int delta = drawingFactor / 2;
            ...
            int colorN = 0;
            foreach (Task<List<Tree>> routeTask in routes)
            {
                List<Tree> route = await routeTask;
                int p1x = route[0].x * drawingFactor + delta;
                int p1y = route[0].y * drawingFactor + delta;
                Color color = cvalues[colorN % cvalues.Length];
                Pen pen = new Pen(color, 1);
            ...
            bm.Save(Path.Combine(path, woodID.ToString() + "_escapeRoutes.jpg"), ImageFormat.Jpeg);
            Console.ForegroundColor = ConsoleColor.Yellow;
            Console.WriteLine($"ASYNC | {woodID}:write bitmap routes {woodID} end");
        }
        public async void AsyncWriteWoodToDB()
        {
            Console.ForegroundColor = ConsoleColor.Green;
            Console.WriteLine($"ASYNC | {woodID}:write db wood {woodID} start");
            List<DBWoodRecord> records = new List<DBWoodRecord>();
            foreach (Tree t in trees)
            {
                records.Add(new DBWoodRecord(woodID, t.treeID, t.x, t.y));
            }
            db.AsyncWriteWoodRecords(records);
            Console.ForegroundColor = ConsoleColor.Green;
            Console.WriteLine($"ASYNC | {woodID}:write db wood {woodID} end");
        }
        public async Task<List<Tree>> AsyncEscapeMonkey(Monkey monkey)
        {
            ...
            do
            {
                ...
                if (distanceToMonkey.Count == 0)
                {
                    await Task.Run(() => AsyncwriteRouteToDB(monkey, route));
                    Console.ForegroundColor = ConsoleColor.White;
                    Console.WriteLine($"{woodID}:end {woodID},{monkey.name}");
                    return route;
                }
